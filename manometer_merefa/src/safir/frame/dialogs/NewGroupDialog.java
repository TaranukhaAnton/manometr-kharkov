 ...
 * Created on 8 Марта 2008 г., 14:53
 */
...
public class NewGroupDialog extends JDialog {
	...
	public NewGroupDialog(SafirFrame parent) {
	...
	private void initComponents() {
		...
		cancelButton.setLocation(50, 350);
		add(cancelButton);
		createTable();
	...
	private void button1ActionPerformed(ActionEvent evt) {
		Group newGroup = new Group(); // используем конструктор без параметров
		// поле номер группы заполнится автоматически
		// !!!
	//	DefaultTreeModel model = (DefaultTreeModel) tree.getModel();
		//DefaultMutableTreeNode node = (DefaultMutableTreeNode) 

		Termocam termocam = (Termocam)parent.clickedPath.getLastPathComponent();
	...
	//	newGroup.setPressures(pressures);
		//newGroup.setT23(ind);
		SensorType st = SensorTypes.INSTANSE.sensorTypes.get(choice1
				.getSelectedIndex());

		if (st.type == 2) {
			int baseRow = pressures.indexOf(Double.parseDouble(choice2.getSelectedItem()));
		...
		//	DefaultMutableTreeNode newNode = new DefaultMutableTreeNode(
			...
			((DefaultTreeModel) tree.getModel()).insertNodeInto(newGroup, termocam, termocam.getChildCount());
	...
	private void createTable() {
		table = new JTable(new MyTableModel(1, 1));
	...
	private void refreshTable() {
		...
		((MyTableModel) table.getModel()).setRowCount(pointP + 1);

		JList rowHeader = new JList(new MyListModel(rowHeaderNames));
		...
		int count = upperHeaders.size() - cm.getColumnCount();
		if (count > 0) {
		...
		for (String col : upperHeaders) {
			columns.nextElement().setHeaderValue(col);
		}
		TableColumn col2 = new TableColumn();
		cm.addColumn(col2);
		cm.removeColumn(col2);
	}
}
...
class MyTableModel extends AbstractTableModel {
	...
	public MyTableModel(int x, int y) {
		this.x1 = x;
		this.y1 = y;
	}
...
class MyListModel extends AbstractListModel {
	 ...
	 * 
	 */
	public MyListModel(LinkedList<String> headers) {
