...
public class CopyProgressDialog extends JDialog {
  ...
  private FileElement toDir=null;
  ...
  public void addFileToCopy(FileElement el){
    filesToCopy.addElement(el);    
  } 
  
  public void setDirToCopy(FileElement el){
    toDir=el;
  }
  ...
  private void copyDir(File dir,File dest) throws CopyCancelledException{
  ...
  private void copyFile(File fin,File fout) throws CopyCancelledException {
  ...
  private void copyFiles() throws CopyCancelledException{
    ...
    for(int i=0;i<filesToCopy.size();i++){
      FileElement el=(FileElement)filesToCopy.elementAt(i);
      File destFile=new File(toDir.getPath()+(toDir.getPath().endsWith(File.separator)?"":File.separator)+el.getName());
      if(el.isDirectory()){
