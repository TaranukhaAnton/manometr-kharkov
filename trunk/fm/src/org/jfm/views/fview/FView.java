...
public class FView extends JTextArea {
  private FileElement el;
  
  public FView() {
    this(null,false);
  }
  
  public FView(FileElement el,boolean editable){
    setEl(el);
    setEditable(editable);
  }
  
  
  public FileElement getEl() {
    return this.el;
  }
  
  private void calculateDirInfo(File f,DirInfo info){
    if(!f.isDirectory()){
        return;
    }
  ...
  public void setEl(FileElement setEl) {
    this.el = setEl;
    ...
    try{
   // String type=el.toURL().openConnection().getContentType();
    //System.err.println("content-type:"+type);
    StringBuffer text=new StringBuffer((int)el.length());
    java.io.FileInputStream in=new java.io.FileInputStream(el);
