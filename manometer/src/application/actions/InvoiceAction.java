...
import application.utils.amount.JAmount;
import application.utils.amount.JAmountEN;
import application.utils.amount.JAmountRU;
import application.utils.amount.JAmountUA;
...
//        System.out.println(invoice.getShipments().size()+"  ");
    ...
    public ActionForward verifyDivideInvoiceForm(ActionMapping mapping, ActionForm form,
                                                 HttpServletRequest request, HttpServletResponse response) {
        // printProps(request);
        DateFormat f = new SimpleDateFormat("dd.MM.yyyy");
        ...
        try {
            Boolean isInvoice1 = new Boolean(request.getParameter("isInvoice1"));
            final String number11 = request.getParameter("number1");
            String numberModifier1 = request.getParameter("numberModifier1");
            String employerShortName1 = request.getParameter("employer1");
            Date date1 = f.parse(request.getParameter("date1"));

            Boolean isInvoice2 = new Boolean(request.getParameter("isInvoice2"));
            final String number21 = request.getParameter("number2");
            String numberModifier2 = request.getParameter("numberModifier2");
            String employerShortName2 = request.getParameter("employer2");
            Date date2 = f.parse(request.getParameter("date2"));

            Boolean correct = true;
            String mes = "";
            String resp = "{";
            if (StringUtils.isNumeric(number11) && StringUtils.isNotBlank(number11)) {
                Integer number1 = new Integer(number11);
                boolean isInvoice1Presence = isInvoicePresence(isInvoice1, number1, numberModifier1, date1);
                if (isInvoice1Presence) {
                    correct = false;
                    mes += ((isInvoice1) ? "Cчет" : "Кп") + " 1 уже существует. \\u000D";
                }
                resp += "\"presence1\":" + !isInvoice1Presence + ",";
                resp += "\"number1\":" + true + ",";
            } else {
                mes += ((isInvoice1) ? "Cчет" : "Кп") + " 1 -- параметр «Номер» должен быть заполнен. \\u000D";
                resp += "\"presence1\":" + true + ",";
                resp += "\"number1\":" + false + ",";
                correct = false;
            }
            Customer employer1 = Factory.getCustomerDAO().findByShortName(employerShortName1);
            boolean employer1presence = employer1 != null;
            resp += "\"employer1\":" + employer1presence + ",";
            if (!employer1presence) {
                correct = false;
                mes += "Неверно указан заказчик 1. \\u000D ";
            }
            if (StringUtils.isNumeric(number21) && StringUtils.isNotBlank(number21)) {
                Integer number2 = new Integer(number21);
                boolean isInvoice2Presence = isInvoicePresence(isInvoice2, number2, numberModifier2, date2);
                if (isInvoice2Presence) {
                    correct = false;
                    mes += ((isInvoice2) ? "Cчет" : "Кп") + "Cчет/кп 2 уже существует. \\u000D";
                }
                resp += "\"presence2\":" + !isInvoice2Presence + ",";

                Customer employer2 = Factory.getCustomerDAO().findByShortName(employerShortName2);
                boolean employer2presence = employer2 != null;
                resp += "\"employer2\":" + employer2presence + ",";
                if (!employer2presence) {
                    correct = false;
                    mes += "Неверно указан заказчик 2. \\u000D ";
                }
            } else {
                resp += "\"presence2\":true,\"employer2\":true,";
            }
    ...
    private boolean isInvoicePresence(Boolean isInvoice, Integer number, String numberModifier, Date date) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("isInvoice", isInvoice);
        map.put("number", number);
        map.put("numberModifier", numberModifier);
...
//            System.out.println(" invoices.size() " + invoices.size());
        if (invoices.size() > 0) {
            for (Invoice inv : invoices) {
                if (inv.getDate().getYear() == date.getYear()) {
                    return true;
                }
            }
        }
            ...
            if (invoices.size() > 0) {
                for (Invoice inv : invoices) {
                    if (inv.getDate().getYear() == f.parse(request.getParameter("date")).getYear()) {
                        correct = false;
                        mes += ((isInvoice) ? "Cчет" : "Кп") + " уже существует. \\u000D";
                        break;
                    }
                }
            }
...
//            invoice.setExchangeRate(new BigDecimal("1"));
            ...
            List<Supplier> suppliers = Factory.getSupplierDAO().findListByParameter("default_row", true);
                if (suppliers.isEmpty())

             suppliers = Factory.getSupplierDAO().findAll();
            if (!suppliers.isEmpty()) {
                invoice.setSupplier(suppliers.get(0));
                invoice.setExchangeRate(suppliers.get(0).getCurrency().getExchangeRate());
            }  else {
                System.err.println("Беда. Нет поставщиков.");
            }
    ...
    public ActionForward copyInvoice(ActionMapping mapping, ActionForm form,
        ...
        try {
            Invoice parent = Factory.getInvoiceDAO().findById(new Long(request.getParameter("parentID")));
            final String employerShortName = request.getParameter("emploer");
    ...
    public ActionForward divideInvoice(ActionMapping mapping, ActionForm form,
                                       HttpServletRequest request, HttpServletResponse response) {
        printProps(request);
        try {
            DateFormat f = new SimpleDateFormat("dd.MM.yyyy");
            ...
            final String employerShortName1 = request.getParameter("employer1");
            final String consumerShortName1 = request.getParameter("consumer1");
            final Date date1 = f.parse(request.getParameter("date1"));
            final Boolean isInvoice1 = new Boolean(request.getParameter("isInvoice1"));
            final Integer number1 = new Integer(request.getParameter("number1"));
            final String numberModifier1 = request.getParameter("numberModifier1");

            final String employerShortName2 = request.getParameter("employer2");
            final String consumerShortName2 = request.getParameter("consumer2");
            final Date date2 = f.parse(request.getParameter("date2"));
            final Boolean isInvoice2 = new Boolean(request.getParameter("isInvoice2"));
            final Integer number2 = new Integer(request.getParameter("number2"));
            final String numberModifier2 = request.getParameter("numberModifier2");

            Invoice newInvoice1 = createCopyOfInvoice(parent, employerShortName1, consumerShortName1, date1, isInvoice1, number1, numberModifier1);
            createCopyOfInvoice(parent, employerShortName2, consumerShortName2, date2, isInvoice2, number2, numberModifier2);
            ActionForward actionForward = new ActionForward();
            actionForward.setPath("/invoiceAction.do?method=viewInvoice&id=" + newInvoice1.getId());
            actionForward.setRedirect(true);
            return actionForward;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Invoice createCopyOfInvoice(Invoice parent, String employerShortName, String consumerShortName, Date date, Boolean invoice, Integer number, String numberModifier) {
        ...
        Customer employer = Factory.getCustomerDAO().findByShortName(employerShortName);
        Customer consumer = Factory.getCustomerDAO().findByShortName(consumerShortName);

        newInvoice.setEmploer(employer);
    ...
    public ActionForward viewInvoice(ActionMapping mapping, ActionForm form,
        ...
        final List<Invoice> invoices = (u == null) ? Factory.getInvoiceDAO().findAll() : (u.getInvoiceFilter() != null) ? u.getInvoiceFilter().doFilter() : Factory.getInvoiceDAO().findAll();
...
//            JasperReport report = JasperCompileManager.compileReport(context.getRealPath("/disign/invoice2.jrxml"));
            ...
            final String language = invoice.getSupplier().getLanguage();
            String filePath;
            String cityName;
            String orgForm;
            String emploer;
            JAmount jAmount;
            String pathToHeader = context.getRealPath("/images/reportImages/") + "\\header";


            final City city = Factory.getCityDAO().findById(invoice.getEmploer().getCity());
            if ("en".equals(language)) {
                filePath = "/disign/invoice_en.jrxml";
                jAmount = new JAmountEN();
                cityName = Customer.localityTypeAliasEn[invoice.getEmploer().getLocalityType().intValue()];
                cityName += " " + city.getNameEn();
                orgForm = invoice.getEmploer().getOrgForm().getNameEng();
                emploer = invoice.getEmploer().getName();

                pathToHeader += "_en";
            } else if ("ua".equals(language)) {
                filePath = "/disign/invoice_ua.jrxml";
                jAmount = new JAmountUA();
                cityName = Customer.localityTypeAliasUkr[invoice.getEmploer().getLocalityType().intValue()];

                cityName += " " + city.getNameUkr();
                orgForm = invoice.getEmploer().getOrgForm().getNameUkr();
                emploer = invoice.getEmploer().getNameUkr();

                pathToHeader += "_ua";
            } else {
                filePath = "/disign/invoice_ru.jrxml";
                jAmount = new JAmountRU();
                cityName = Customer.localityTypeAlias[invoice.getEmploer().getLocalityType().intValue()];
                cityName += " " + city.getName();
                orgForm = invoice.getEmploer().getOrgForm().getName();
                emploer = invoice.getEmploer().getName();
                pathToHeader += "_ru";
            ...
            JasperReport report = JasperCompileManager.compileReport(context.getRealPath(filePath));
...
//            parameters.put("emploer", invoice.getEmploer().getName());
            parameters.put("emploer", emploer);
            ...
            parameters.put("paymentTerms", jAmount.getAmount(currencyId, invoice.getTotal()));
            parameters.put("deliveryTerms", jAmount.getAmount(currencyId, invoice.getTotal()));
            parameters.put("delivery", jAmount.getAmount(currencyId, invoice.getTotal()));
...
//            parameters.put("path", context.getRealPath("/images/reportImages/"));
            ...
            if (type.equals("xls")) {
                ...
                jrXlsExporter.setParameter(JRXlsExporterParameter.OUTPUT_STREAM, out);
